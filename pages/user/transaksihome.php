...
$result_count = mysqli_query($conn, "select count(*) from transaksi where nasabah_id = {$_SESSION['id']}");
$total_rows = mysqli_fetch_array($result_count)[0];
$total_pages = ceil($total_rows / $no_of_records_per_page);

$result = mysqli_query($conn, "select t.*, n.nama from transaksi as t inner join nasabah as n on t.nasabah_id=n.id where t.nasabah_id={$_SESSION['id']} order by t.id desc limit $offset, $no_of_records_per_page") or die(mysqli_error($conn));
                                            ...
                                            <tbody>
                                                <?php foreach ($data_transaksi as $transaksi) { ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a href=<?php echo "../printnasabah.php?id={$transaksi['id']}" ?>><button class="btn btn-danger">Print</button></a>
                                                        </td>
                                                    </tr>
                                                <?php } ?>
                                            </tbody>
